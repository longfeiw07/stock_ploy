...
# engine = create_engine('mysql+pymysql://root:nopwd@mysql@localhost:3306/MySQL80/tushare')
...
def db_tick_data(start,end):
    ...
    for day in getDateIterator(start, end):
        ...
        tick_day_data.to_sql(table_name, engine, index= False, if_exists='append')
def db_daily_basic(start, end):
    ...
    for day in getDateIterator(start, end):
        ...
        daily_basic.to_sql(table_name, engine, index= False, if_exists='replace')
def db_top_list(start, end):
    ...
    for day in getDateIterator(start, end):
        ...
        top_lst.to_sql(table_name, engine, index= False, if_exists='replace')
def db_oneyflow(start, end):
    ...
    for day in getDateIterator(start, end):
        ...
        moneyflow.to_sql(table_name, engine, index= False, if_exists='replace')
def get_stock_basic(fields=None,chunksize=None):
    ...
    stock_basic = pd.read_sql(sql=sql_cmd, con=engine, columns=fields, chunksize=chunksize)
    return stock_basic
...
def test():
    try:
        engine = create_engine('mysql+pymysql://root:nopwd@mysql@localhost:3306/tushare') 
        sql_cmd = "SELECT ts_code, symbol, name FROM stock_basic"
        fields = ['ts_code', 'symbol', 'name']
        stock_basic = pd.read_sql(sql=sql_cmd, con=engine, columns=['ts_code', 'symbol', 'name'], chunksize=None)
        print(stock_basic)
    except Exception as e:
        print('error:',e)
...
if __name__ == "__main__":    
    ...
    db_tick_data('20190507', '20190601')
    # getDaily_basic('20190603', '20190603')
    # readDaily_basic('20190506')
    # test()
    # getTop_list('20190501', '20190601')
